...
- 进货入库页替换占位页面，支持日期范围和商品名称筛选。
- 新增入库单支持一次录入多个商品明细。
- 入库保存写入 `purchase_records`、`purchase_items`、`stock_batches`。
- 入库后自动增加商品当前库存，并按移动加权平均成本更新库存均价。
- 每条入库明细生成一条库存批次，支持生产日期、到期日期和批次号。
- 支持查看入库单只读详情。
- 兼容已有 SQLite 数据库，启动时通过迁移补齐入库和批次表新字段。

## 下一步建议

第五阶段建议进入销售记账基础能力：收银单、多商品明细、应收/成本/利润计算、库存扣减和库存不足提示。盘点、赊账和报表仍建议继续分阶段推进。
