...
“小铺掌柜”是面向家庭小卖铺的本地离线桌面软件。当前仓库已完成第二阶段：项目骨架、SQLite 初始化脚本、主窗口导航、首次使用初始化、6 位 PIN 登录、恢复密钥重置 PIN，以及系统设置页基础配置。
...
第三阶段建议进入商品管理基础能力：分类列表、商品新增/编辑/停用、默认售价、库存预警、是否启用保质期。销售、入库和报表仍建议继续分阶段推进。
